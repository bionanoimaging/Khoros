%kkbitlshift 'Bitwise LEFT SHIFT of Input 1 by (Input 2 or Constant) Bits'
% This MatLab function was automatically generated by a converter (KhorosToMatLab) from the Khoros kbitlshift.pane file
%
% Parameters: 
...
%
% Example: o = kkbitlshift({i1, i2}, {'i1','';'o','';'i2','';'real',0})
%
% Khoros helpfile follows below:
%
%  PROGRAM
% kbitlshift - Bitwise LEFT SHIFT of Input 1 by (Input 2 or Constant) Bits
%
%  DESCRIPTION
% The \fBLeft Shift" operator performs a bitwise left shift on each point in
% \fBInput 1" by the number of bits defined by either the corresponding
% point in \fBInput 2" or the \fBConstant\fP value, which
% ever is specified by the user.  Zero's are shifted into the
% least significant bits.  In effect each data value is multiplied by two.
% 
...
% 
% Executing \fBLeft Shift" runs the program "kbitwise\fP with the -ls flag.
%
%  
%
%  EXAMPLES
...
function varargout = kkbitlshift(varargin)
if nargin ==0
  Inputs={};arglist={'',''};
elseif nargin ==1
...
else error('Usage: [out1,..] = kkbitlshift(Inputs,arglist).');
end
if size(arglist,2)~=2
...
[varargout{:}]=callKhoros([w 'kbitwise"  -ls'],Inputs,narglist);
