%kcmaxlik_printsig 'Print a Maximum Likelihood Signature '
% This MatLab function was automatically generated by a converter (KhorosToMatLab) from the Khoros cmaxlik_printsig.pane file
%
% Parameters: 
...
%
% Example: o = kcmaxlik_printsig(isig, {'isig','';'omode',1;'o',''})
%
% Khoros helpfile follows below:
%
%  PROGRAM
% cmaxlik_printsig - Print a Maximum Likelihood Signature
%
%  DESCRIPTION
% This operator prints out the contents of a Maximum Likelihood Signature file created with "cmaxlik_signature". It expects a valid Maximum Likelihood signature file (passed with the parameter [-isig]), so its input must be either the output of a \fIcmaxlik_signature\fP or the output of \fIkappend\fP which appended several signatures together in the time dimension. Its output (passed with the parameter [-o], or redirected to kstdout) can be formatted so it can be used in reports, etc.
...
%
%  EXAMPLES
% All examples for the Classify toolbox are listed on the Classify Toolbox Manual. For an example of this routine, please see the example workspace Classify:workspaces:MAXLIK-Signatures.
%
%  "SEE ALSO"
% cmaxlik_signature
%
%  RESTRICTIONS 
% Expect the input to be a valid signature file for the Maximum Likelihood classifier.
%
%  REFERENCES 
...
function varargout = kcmaxlik_printsig(varargin)
if nargin ==0
  Inputs={};arglist={'',''};
elseif nargin ==1
...
else error('Usage: [out1,..] = kcmaxlik_printsig(Inputs,arglist).');
end
if size(arglist,2)~=2
...
[varargout{:}]=callKhoros([w 'cmaxlik_printsig"  '],Inputs,narglist);
