%kminvert 'Compute inverse matrix '
% This MatLab function was automatically generated by a converter (KhorosToMatLab) from the Khoros minvert.pane file
%
% Parameters: 
% InputFile: i 'Input Matrix', required: 'Input matrix'
% Toggle: diag 'Invert Diagonal Only', default: 0: 'Invert diagonal only'
% OutputFile: o 'Output Matrix', required: 'Output matrix'
%
% Example: o = kminvert(i, {'i','';'diag',0;'o',''})
%
...
%  DESCRIPTION
% .I minvert
% computes the inverse of a matrix. If the -diag flag "is\fR selected, then only
% the diagonal elements are inverted (this will work for any size of matrix).
% If the -diag flag is "not\fR selected, then the matrix must be square, and
% the inverse will be computed by backsolving an LU decomposition.
% 
% "minvert\fR uses the LAPACK routines DGETRF, DGETRI, ZGERF, and ZGTRI
...
%
%  RESTRICTIONS 
% "minvert\fR does \fBnot\fR check for a divide by zero condition when computing the
...
function varargout = kminvert(varargin)
if nargin ==0
  Inputs={};arglist={'',''};
elseif nargin ==1
...
else error('Usage: [out1,..] = kminvert(Inputs,arglist).');
end
if size(arglist,2)~=2
  error('arglist must be of form {''ParameterTag1'',value1;''ParameterTag2'',value2}')
 end
narglist={'i', '__input';'diag', 0;'o', '__output'};
maxval={0,0,0};
minval={0,0,0};
istoggle=[0,1,0];
was_set=istoggle * 0;
paramtype={'InputFile','Toggle','OutputFile'};
...
[varargout{:}]=callKhoros([w 'minvert"  '],Inputs,narglist);
