%kkgt 'IF Input 1 is Greater Than (Input 2 or Constant), Output = TRUE'
% This MatLab function was automatically generated by a converter (KhorosToMatLab) from the Khoros kgt.pane file
%
% Parameters: 
...
%
% Example: o = kkgt({i1, i2}, {'i1','';'o','';'i2',''})
%
% Khoros helpfile follows below:
%
%  PROGRAM
% kgt - IF Input 1 is Greater Than (Input 2 or Constant), Output = TRUE
%
%  DESCRIPTION
% The GREATER THAN comparison operator compares each point 
% in \fBInput 1" either with the corresponding point in \fBInput 2\fP 
% or with the \fBConstant" value, which ever is specified by the user. If the 
% value from \fBInput 1" is greater than the value from \fBInput 2\fP 
% or the \fBConstant", the corresponding point in \fBOutput\fP will be assigned 
% the value specified by \fBTRUE Value".  If the value from \fBInput 1\fP 
% is not greater than the value against which it is being compared, the 
% corresponding point in \fBOutput" will be the value specified by \fBFALSE 
% Value". 
% 
...
% 
% Executing the Greater Than operator runs the program "kcompare" with 
% the -gt flag.
%
%  
%
...
function varargout = kkgt(varargin)
if nargin ==0
  Inputs={};arglist={'',''};
elseif nargin ==1
...
else error('Usage: [out1,..] = kkgt(Inputs,arglist).');
end
if size(arglist,2)~=2
...
[varargout{:}]=callKhoros([w 'kcompare"  -gt'],Inputs,narglist);
